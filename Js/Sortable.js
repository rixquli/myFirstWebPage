...
var caseType = ["P", "BR", "A", "INPUT", "DIV", "UL", "OL", "DL", "LI"];
...
function traductionHTML() {
  const allAddObject = example4Right.childNodes;
  var HTMLc = "";
  for (const enfant of allAddObject) {
    for (var i = 0; i < caseType.length; i++) {
      if (enfant.textContent == caseType[i]) {
        var HTMLc = HTMLc + `&lt${caseType[i]}&gt <br>`;
        // if (caseType[i] == "BR") {
        //   var HTMLcode = `<${caseType[i]}>`;
        // } else {
          let str = caseType[i].innerHTML;
          let start = str.indexOf('id="') + 4;
          let stop = str.indexOf('"', start);
          let id = str.substring(start, stop);
          var HTMLcode = `<${caseType[i]}>${id}</${caseType[i]}>`;
        // }
        break;
      }
    }
  }
  var WEBcode = `&lt;!DOCTYPE html&gt; <br>
  &lt;html lang=&quot;fr&quot;&gt; <br>
  &lt;head&gt;<br>
  ...
  // </body>
  // </html>`;
  var WEBcode2 = HTMLcode;
  document.querySelector("#traduction").innerHTML = WEBcode;
  document.querySelector(".WEB").innerHTML = WEBcode2;
}

var allTextP = document.querySelectorAll(".textP");
for (const textP of allTextP) {
  textP.addEventListener("change", text);
}
function text() {
  alert("test text");
}
  ...
  Sortable.prototype =
    /** @lends Sortable.prototype */
    {
      ...
      _onDrop: function _onDrop(
        ...
        const allAddObject = example4Right.childNodes;
        for (const enfant of allAddObject) {
          if (enfant.innerHTML.includes("onlyDraggable") == false) {
            enfant.innerHTML = `<div class="onlyDraggable">${enfant.innerHTML}<input type="image" class="settingButton" src="../ressource/126472.png"/><input type="button" class=".buttonRight" value="X" onClick="deleting(this.parentNode.parentNode)"/></div>`;
          }
        }
        // traductionHTML();
        this._nulling();  
