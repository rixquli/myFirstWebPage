const selectElement = document.getElementById("recitheo");

selectElement.addEventListener("change", function () {
  if (this.value == "reciproque") {
    window.location.href = `reciproque.html`;
  }
});
...
function triangle(un, deux, resultat) {
  ...
  if (un < resultat) {
    ...
    // si un est plus petit que deux alors de l'hypot??nuse
    ctx.lineTo(25, un * 25);
    ctx.lineTo(resultat * 25, un * 25);
  } else {
    // sinon un est l'hypot??nuse
...
function phraseReponse(un, deux, resultat) {
  ...
  D'apres le théoreme de Pythagore : </br>
  AC2= AB2+BC2 </br>
  AC2= ${un} 2+${deux} 2 </br>
